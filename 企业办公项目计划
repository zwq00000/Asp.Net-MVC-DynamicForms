...
对于一个创意产品来说，没有直接的用户不等于没有潜在用户，那么谁来代表这些潜在用户提出需求呢？
我们需要指派一个人，他**能够代表潜在的用户**，为开发团队提供应用场景的定义和描述，对开发团队提出功能要求，和开发团队讨价还价，能够和潜在用户进行沟通，组织潜在用户试用产品。
这个任务是不是很艰巨呢？所以一个好的**产品经理**做到极致就是乔布斯，能够创造一个伟大的产品，做不好的就比比皆是，比如那些在网上天天挨骂的产品。
一个项目总要有成本/时间的限制，如果事先不考虑这些，项目就会永无止境，落入*焦油坑*中。**回想起来要感谢那些当初很不理解的甲方，实在是他们在做着一些非常人所能为的艰巨事情。**
...
如果你一直看到了这里，就可以说说你最关心的事情。
所谓插件结构、系统框架，这个东西是想不明白的，就如同当年看 Eclipse 的插件管理，什么叫做扩展点，扩展点里写什么内容，如何定义自己的扩展点。
如果一直用各种控件进行编程，这个问题永远也想不明白。一个功能结构就会有一个功能的数据模型，这个模型用来描述这个功能的特征，模型根据这个数据结构进行初始化，或者根据提供的主要参数进行初始化。扩展点就是自定义的功能模型的标准接口。那么现有扩展点还是现有功能模型，当然必须是现有功能模型，功能模型支持外部配置文件，然后才能说支持扩展点方式初始化。
如果看一个插件模型的基本结构，就是一个启动器，启动器包含启动、暂停、恢复和结束/卸载几个方法。这和 Windows Service 的结构很类似，在 Linux 里面只有 启动和停止两个状态，restar 就是先停止再启动而已。具体启动功能做些什么事情，是由功能所引用的资源决定的。
这个结构可能连设计模式都谈不上，就是一个接口而已。
