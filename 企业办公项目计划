...
所谓插件结构、系统框架，这个东西是想不明白的，就如同当年看 Eslipse 的插件管理，什么叫做扩展点，扩展点里写什么内容，如何定义自己的扩展点。
...
如果看一个插件模型的基本结构，就是一个启动器，启动器包含启动、暂停、恢复和结束/卸载几个方法。和 Windows Service 的结构很类似。具体启动功能做些什么事情，是由功能所引用的资源决定的。
