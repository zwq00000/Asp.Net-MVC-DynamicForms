...
这种方法具备相当的灵活性，可以容纳数据表的变化，并且能够动态绑定表示层。
 - DataSet 和XML数据本身是相通的，即可以从 DataSet转换为XML ，同时也可以由DataSet 对象读取 XML数据完成数据集填充。
...
##二、动态 Web 表单 ##
动态 Web 表单是指没有严格规定数据关系的设计，即无模式**（Schema-Less）**
一个基本差异是面向文档的数据库不在文档之间规定严格的关系。 这有助于维护它们的非模式化设计。
现行系统虽然不是严格意义上的无模式（具有明确的数据库关联关系），但是动态表单的数据定义方式使得无法采用强类型对象和查询结构来实现。
无模式的意义是可以设计松散的基于文档的处理方式，便于生产环境中动态调整表单设计。
无模式的缺点是，一套逻辑完备的对象模型表示了很多潜在含义，这些都需要用一种方法进行描述。
这里产生了一个分歧点，即动态不一定等同于无模式，因为每一个版本都是完整的解决功能，只是我们不愿意去一个一个开发而已。
如果我们实现了多版本部署的问题，是不是能够解决这个事情呢？

一个完整的 Asp.Net MVC 表单处理包括几个部分， Model-Controler-Viewer 负责联系的是Model，Controler 任务最重，要负责查询参数处理，和处理用户提交的表单数据，实体化的表单框架提供了自动绑定功能，把 Request.Params 字典内容填充到实体对象中，
这部分工作比较繁琐，类型化的实体对象实现了自动处理，这是多么令人振奋的事情呀。无模式的构造方法则完全不能享用这个强大的功能。

处理流程是 接受查询请求-->提取数据/准备实体对象-->绑定到视图-->用户填写并提交-->根据绑定规则重建实体对象-->持久化(保持到数据库)

这种模式化的解决方案可能会影响灵活性。

表单模板的问题不是最主要的，提交数据的服务器端验证和处理比较棘手
...
1. Web表单模板的本质
   Web表单模板本质是一个 Html 片段,表述页面表单部分的基本布局和文档部件属性。
   可以包含局部样式，可以包含事件处理脚本，尽量保持简单以便于更好的和页面整体相容。
   很多页面模板需要处理整个 Html 页面的内容，所以通常功能比较复杂，表单模板只是页面的一部分（类似 Aspx 模板页的 ContentContainer 中包含的内容）。
   是否实现服务器端数据绑定是一个比较现实的问题，如果在客户端绑定数据，那么这个模板可以直接嵌入到主页面中，甚至不需要计算，就当做一个普通的 Html 片段处理就好了。

2. 表单模板按照多版本并行和功能要求会有多个版本共存，我们关心的问题是
 - 模板处理机制
